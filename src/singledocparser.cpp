...
void SingleDocParser::HandleBlockSequence(EventHandler& eventHandler) {
  ...
  while (1) {
    if (m_scanner.empty())
      ParserException(m_scanner.mark(), ErrorMsg::END_OF_SEQ);

    Token token = m_scanner.peek();
    if (token.type != Token::BLOCK_ENTRY && token.type != Token::BLOCK_SEQ_END)
      ParserException(token.mark, ErrorMsg::END_OF_SEQ);
...
void SingleDocParser::HandleFlowSequence(EventHandler& eventHandler) {
  ...
  while (1) {
    if (m_scanner.empty())
      ParserException(m_scanner.mark(), ErrorMsg::END_OF_SEQ_FLOW);
    ...
    if (token.type == Token::FLOW_ENTRY)
      ...
      ParserException(token.mark, ErrorMsg::END_OF_SEQ_FLOW);
  }
...
void SingleDocParser::HandleBlockMap(EventHandler& eventHandler) {
  ...
  while (1) {
    if (m_scanner.empty())
      ParserException(m_scanner.mark(), ErrorMsg::END_OF_MAP);

    Token token = m_scanner.peek();
    if (token.type != Token::KEY && token.type != Token::VALUE &&
        token.type != Token::BLOCK_MAP_END)
      ParserException(token.mark, ErrorMsg::END_OF_MAP);
...
void SingleDocParser::HandleFlowMap(EventHandler& eventHandler) {
  ...
  while (1) {
    if (m_scanner.empty())
      ParserException(m_scanner.mark(), ErrorMsg::END_OF_MAP_FLOW);
    ...
    if (nextToken.type == Token::FLOW_ENTRY)
      ...
      ParserException(nextToken.mark, ErrorMsg::END_OF_MAP_FLOW);
  }
...
void SingleDocParser::ParseTag(std::string& tag) {
  Token& token = m_scanner.peek();
  if (!tag.empty())
    ParserException(token.mark, ErrorMsg::MULTIPLE_TAGS);
...
void SingleDocParser::ParseAnchor(anchor_t& anchor) {
  Token& token = m_scanner.peek();
  if (anchor)
    ParserException(token.mark, ErrorMsg::MULTIPLE_ANCHORS);
...
anchor_t SingleDocParser::LookupAnchor(const Mark& mark,
  ...
  if (it == m_anchors.end())
    ParserException(mark, ErrorMsg::UNKNOWN_ANCHOR);
