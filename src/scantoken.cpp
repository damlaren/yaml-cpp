...
void Scanner::ScanFlowEnd() {
  if (InBlockContext())
    ParserException(INPUT.mark(), ErrorMsg::FLOW_END);
  ...
  if (m_flows.top() != flowType)
    ParserException(mark, ErrorMsg::FLOW_END);
  m_flows.pop();
...
void Scanner::ScanBlockEntry() {
  ...
  if (InFlowContext())
    ParserException(INPUT.mark(), ErrorMsg::BLOCK_ENTRY);
...
void Scanner::ScanKey() {
  ...
  if (InBlockContext()) {
    if (!m_simpleKeyAllowed)
      ParserException(INPUT.mark(), ErrorMsg::MAP_KEY);
...
void Scanner::ScanValue() {
  ...
  if (isSimpleKey) {
    ...
    if (InBlockContext()) {
      if (!m_simpleKeyAllowed)
        ParserException(INPUT.mark(), ErrorMsg::MAP_VALUE);
...
void Scanner::ScanAnchorOrAlias() {
  ...
  if (name.empty())
    ParserException(INPUT.mark(), alias ? ErrorMsg::ALIAS_NOT_FOUND
                                              : ErrorMsg::ANCHOR_NOT_FOUND);

  // and needs to end correctly
  if (INPUT && !Exp::AnchorEnd().Matches(INPUT))
    ParserException(INPUT.mark(), alias ? ErrorMsg::CHAR_IN_ALIAS
...
void Scanner::ScanPlainScalar() {
  ...
  //	ParserException(INPUT.mark(), ErrorMsg::CHAR_IN_SCALAR);
...
void Scanner::ScanBlockScalar() {
  ...
  if (INPUT && !Exp::Break().Matches(INPUT))
    ParserException(INPUT.mark(), ErrorMsg::CHAR_IN_BLOCK);
